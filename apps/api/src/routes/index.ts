...
export async function registerRoutes(app: FastifyInstance) {
  ...
  app.post('/admin/production/validation/gates/:gateId', async (req) => {
    ...
    return recordProductionGate({ gateId: (req.params as { gateId: string }).gateId, ...body, actorId: req.adminUser?.userId });
  });
  ...
  app.get('/projects/cards', async (req) => listProjectCards(req.authUser?.userId));
  app.get('/projects/:projectId/dashboard', async (req) => getProjectDashboard((req.params as { projectId: string }).projectId, req.authUser?.userId));
  app.post('/projects/:projectId/favorite', async (req) => { const body = z.object({ favorite: z.boolean().default(true) }).parse(req.body ?? {}); return favoriteProject((req.params as { projectId: string }).projectId, req.authUser?.userId ?? '', body.favorite); });
  app.post('/projects/:projectId/invitations', async (req) => createProjectInvitation({ projectId: (req.params as { projectId: string }).projectId, invitedBy: req.authUser?.userId ?? '', ...z.object({ email: z.string().email(), role: z.enum(['co-admin','collaborator','member','viewer']).default('member'), message: z.string().optional() }).parse(req.body ?? {}) }));
  app.post('/projects/invitations/:invitationId/accept', async (req) => acceptProjectInvitation({ invitationId: (req.params as { invitationId: string }).invitationId, userId: req.authUser?.userId ?? '', displayName: req.authUser?.email ?? 'Project member' }));
